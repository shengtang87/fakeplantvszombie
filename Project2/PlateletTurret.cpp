...
PlateletTurret::PlateletTurret(float x, float y) :
	Turret("play/turret-2.png", x, y,10, Price, 0.5,3) {
...
void PlateletTurret::CreateBullet() {
	...
	getPlayScene()->EffectGroup->AddNewObject(new ShootEffect((Position + normalized * 28).x + 25	, (Position + normalized * 28).y));
...
void PlateletTurret::Update(float deltaTime){
	...
	if (!Target) {
		...
		for (auto& it : scene->EnemyGroup->GetObjects()) {
			...
			if (it->Position.x > Position.x && ey >= ty-1 && ey <= ty +1 || getPlayScene()->gogogo == true) {
